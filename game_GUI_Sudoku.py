from solver_algo import isValid, solve, find_empty
...
class Board:
    ...
    def visualSolve(self, wrong, time):
        ...
        for nums in range(9):
            if isValid(self.board, nums+1, (empty[0],empty[1])):
    ...
    def hint(self, keys):
        ...
        while True: #keeps generating i,j coords until it finds a isValid random spot
                    ...
                    if event.key == pygame.K_9:
                        ...
                        if selected in keyDict:
                            if keyDict[selected] != board.solvedBoard[selected[1]][selected[0]]: #clear tile when incorrect value is inputted
                                ...
                                del keyDict[selected]
                                break
                            # isValid and correct entry into cell
